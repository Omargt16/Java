...
public class DrawTextPanel extends JPanel {
	...
	private class Canvas extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			if (theString != null) {
				for (DrawTextItem value : theString) {
					if (value != null) {
						value.draw(g);
					}
				}
	...
	/**
	 * An object of type MenuHandler is registered as the ActionListener  all thfore
	...
	public void doMousePress(MouseEvent e) {
		...
		s.setRotationAngle(currentRotationAngle);
...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
	...
	public JMenuBar getMenuBar() {
		if (menuBar == null) {
			...
			JMenuItem borderItem = new JMenuItem("Set angle...");
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			...
			// implemented.");
			File file = fileChooser.getOutputFile(this, "Select file", "file.txt");
			int blue, green, red, x, y;
			Color textColor;
			String string;
			...
			try {
				...
				// background
				int blue = Integer.parseInt(sc.nextLine());
				int green = Integer.parseInt(sc.nextLine());
				...
				while (sc.hasNextLine()) {
					...
					textItem = new DrawTextItem(string,x,y);
					textItem.setTextColor(color);
					theString.add(textItem);
				}
		...
		}else if (command.equals("Set angle...")) { 
			int angle = Integer.parseInt( JOptionPane.showInputDialog("Enter the angle"));
